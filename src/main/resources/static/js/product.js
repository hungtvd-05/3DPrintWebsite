...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('productForm').addEventListener('submit', function(e) {
        ...
        if (!stlDropzone || !stlDropzone.files || stlDropzone.files.length === 0) {
...
function showImagePreview(file, serverFilename) {
    ...
    reader.onload = function(e) {
        ...
        div.dataset.filename = serverFilename; // Sử dụng tên file từ server
        div.innerHTML = `
            <div class="preview-content">
                <img src="/tmp/img/${serverFilename}" class="preview-image">
                <button type="button" class="btn-remove" onclick="window.removeFile('${serverFilename}', 'image')"><i class="ti ti-trash"></i></button>
            </div>
        `;
        preview.appendChild(div);
    };
    reader.readAsDataURL(file); // Vẫn dùng FileReader để tạo preview tạm thời
}
...
function showSTLPreview(file, serverFilename) {
    ...
    // Tạo ảnh đại diện thay vì canvas
    ...
    `;
    previewImg.onclick = () => showSTLModal(file, serverFilename);

    // Tạo ảnh loading tạm thời với tỷ lệ 16:9
    ...
    deleteBtn.onclick = () => window.removeFile(serverFilename, 'stl');
    container.appendChild(deleteBtn);

    div.appendChild(container);
    preview.appendChild(div);

    // Tạo ảnh đại diện với kích thước FullHD
    setupSTLViewer(null, file, serverFilename, true);

    // Sau 3 giây, load ảnh đại diện từ server (tăng thời gian chờ vì file lớn hơn)
    setTimeout(() => {
        const img = new Image();
        img.onload = function() {
        ...
        img.onerror = function() {
            ...
            // Retry sau 2 giây nữa
            setTimeout(() => {
                previewImg.src = `/tmp/stl/${serverFilename}_preview.png?t=${Date.now()}`;
            }, 2000);
        };
        img.src = `/tmp/stl/${serverFilename}_preview.png?t=${Date.now()}`;
    }, 3000);
}

function showSTLModal(file, serverFilename) {
    // Tạo modal
...
window.removeFile = function(fileName, type) {
    ...
    if (previewElement) {
        // Dọn dẹp THREE.js renderer nếu là STL
        if (type === 'stl') {
